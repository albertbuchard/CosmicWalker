...
var hidePointSphere = function hidePointSphere(_ref) {
  var index = _ref.index;

  var stateManager = this.stateManager;
  var pointSpheres = stateManager.get('pointSpheres', []);

  if ((0, _experimentJs.hasConstructor)(_experimentJs.Array, pointSpheres) && (0, _experimentJs.hasConstructor)(Number, index) && index < pointSpheres.length && (0, _experimentJs.hasConstructor)(_experimentBabylonJs2.default.Mesh, pointSpheres[index])) {
...
var setViewForDirection = function setViewForDirection(_ref2) {
  var _ref2$direction = _ref2.direction,
      direction = _ref2$direction === undefined ? null : _ref2$direction;
...
var explode = function explode(_ref3) {
  var _ref3$meshes = _ref3.meshes,
      meshes = _ref3$meshes === undefined ? null : _ref3$meshes;
...
var checkIdleAndConnection = function checkIdleAndConnection() {
  ...
  var _ref4 = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : { deferred: new _experimentJs.Deferred(), returnToState: null },
      _ref4$deferred = _ref4.deferred,
      deferred = _ref4$deferred === undefined ? new _experimentJs.Deferred() : _ref4$deferred,
      _ref4$returnToState = _ref4.returnToState,
      returnToState = _ref4$returnToState === undefined ? null : _ref4$returnToState;
...
exports.hidePointSphere = hidePointSphere;
...
window.loadingPromise.then(function () {
  ...
  function loginForm() {
    ...
    var options = { fields: fields, title: 'Create a login', format: 'topCentralSmall', callback: function callback(fields) {
...
function MainScene() {
  ...
  if (R.get.parameters.animation === 'yes') {
    ...
    var camera = scene.initialCamera;
    ...
    var _camera = new _experimentBabylonJs2.default.FreeCamera('OrthoCamera', new _experimentBabylonJs2.default.Vector3(-50, 0, 0), scene);
    _camera.mode = _experimentBabylonJs2.default.Camera.ORTHOGRAPHIC_CAMERA;

    var side = 17;
    _camera.orthoTop = side;
    _camera.orthoBottom = -side;
    _camera.orthoLeft = -side * taskObject.renderSize.width / taskObject.renderSize.height;
    _camera.orthoRight = side * taskObject.renderSize.width / taskObject.renderSize.height;
    //
    //   // This targets the camera to scene origin
    _camera.setTarget(new _experimentBabylonJs2.default.Vector3.Zero());
    //
      ...
      switch (result.code) {
        ...
        default:
          if (parameters.animation === 'yes') {
            _this.stateManager.goToState(R.get.states_running, true);
            return 'animation';
          }
          // Start tutorial
...
var nextTrial = function nextTrial() {
  ...
  if (parameters.animation !== 'yes' && trialIndex === 21) {
    var tutorial = this.R.get.tutorials_getHarder;
    var event = new _experimentJs.EventData(this.R.get.events_goNextPosition);
    this.taskObject.modal({ type: 'centralLarge', title: tutorial.title.en, content: tutorial.content.en, event: event });
...
var goToNextPosition = function goToNextPosition() {
  ...
  if (trialData.currentIndex + parameters.framePerPosition >= positions.length) {
    ...
    if (parameters.animation === 'yes' && state.animationPoints === null) {
      state.animationPoints = (0, _experimentJs.samplePermutation)((0, _experimentJs.rep)(0, positions[0].length).map(function (o, i) {
        return i;
      }).filter(function (e) {
  ...
  stateManager.promise('updatePoints', points).then(function () {
    if (state.animationPoints !== null && stateManager.timeInMs - state.lastHide > 1000 && state.animationPoints.length) {
      var picked = _lodash2.default.random(0, state.animationPoints.length - 1);
    ...
    if (index > parameters.animationStartTurn) {
      _this3.scene.initialCamera.alpha += parameters.angularVelocity;
    }
